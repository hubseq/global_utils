...
alternate input position. Desired argument position of alternate input name for the COMPLETE program call that includes inputs and outputs. Note that if there are several inputs they are considered a single “unit”, so for a command like:
...
def parseRunInput( run_input_arguments ):
    ...
    >>> parseRunInput('mpileup -sampleid=MYSAMPLE -input=s3://bams/my.bam -output="s3://pileup/my.pileup" -pargs="-q 20 -Q 20 -u -f s3://fasta/my.fasta"')
    {'program_name': 'mpileup', 'program_subname': '', 'sampleid': 'MYSAMPLE', 'input': 's3://bams/my.bam', 'output': 's3://pileup/my.pileup', 'pargs': '-q 20 -Q 20 -u -f s3://fasta/my.fasta'}
    ...
    try:
        ...
        while len(rargs_list) > 0:
            if rargs_list[0][0] != '-':
                ...
                run_args_JSON[arg_tuple[0]] = str(arg_tuple[1].lstrip('"').lstrip("'").rstrip('"').rstrip("'"))
...
def createIOJSON( run_args_json ):
    ...
    >>> createIOJSON( {'sampleid': 'MYSAMPLE', 'program_name': 'mpileup', 'program_subname': '', 'input': 's3://bams/my.bam', 'output': 's3://pileup/my.pileup', 'alternate_inputs': 'input1.fasta,input2.bed'} )
    {'input': ['s3://bams/my.bam'], 'output': ['s3://pileup/my.pileup'], 'alternate_inputs': ['input1.fasta', 'input2.bed'], 'alternate_outputs': [], 'program_arguments': '', 'sample_id': 'MYSAMPLE'}
    """
...
def createModuleInstanceJSON( module_template_json, io_json, file_system = 's3' ):
    ...
    >>> io_json = {'input': ['s3://fastq/my.fastq'], 'output': ['s3://align/my.sam'], 'alternate_inputs': ['s3://fasta/input1.fasta', 's3://bed/input2.bed'], 'alternate_outputs': [], 'program_arguments': '', 'sample_id': 'MYSAMPLE'}
    ...
    for pi in module_template_json['program_input']:
        if file_utils.inferFileType(io_json['input']).upper() == pi['input_file_type'].upper():
            mi_json['program_input'] = {'input': file_utils.getFileOnly(io_json['input']),
                                        ...
                                        'input_directory': file_utils.getFileFolder(io_json['input']),
    ...
    for pi in module_template_json['program_output']:
        if file_utils.inferFileType(io_json['output']).upper() == pi['output_file_type'].upper():
            mi_json['program_output'] = {'output': file_utils.getFileOnly(io_json['output']),
                                        ...
                                        'output_directory': file_utils.getFileFolder(io_json['output']),
...
def createProgramArguments( module_instance_json, rtype = 'str', mock = False ):
    ...
    >>> createProgramArguments( mi_json, 'str', True )
    'bwa mem -L s3://bed/input2.bed -o s3://align/my.sam -S -t 4 s3://fasta/input1.fasta s3://fastq/my.fastq'
    """
    ...
    if input_json['input_type'].lower() == 'folder':
        pargs_list = insertArgument(pargs_list, \
                                    [input_json['input_prefix'], file_utils.downloadFolder(input_json['input'], \
                                                                                input_json['input_directory'], \
                                                                                file_utils.inferFileSystem(input_json['input_directory']), \
                                                                                mock)], \
                                    input_json['input_position'])
    else: # input_json['input_type'].lower() == 'file':
        pargs_list = insertArgument(pargs_list, \
                                    [input_json['input_prefix'], file_utils.downloadFiles(input_json['input'], \
                                                                                input_json['input_directory'], \
                                                                                file_utils.inferFileSystem(input_json['input_directory']), \
                                                                                mock)], \
                                     input_json['input_position'])
    ...
    pargs_list = insertArgument(pargs_list, \
                                [output_json['output_prefix'], file_utils.getFullPath(output_json['output_directory'], output_json['output'])], \
    ...
    for alt_input_json in mi_json['alternate_inputs']:
        if alt_input_json['input_type'].lower() == 'folder':
            pargs_list = insertArgument(pargs_list, \
                                        [alt_input_json['input_prefix'], file_utils.downloadFolder(alt_input_json['input'], \
                                                                                        alt_input_json['input_directory'], \
            ...
            pargs_list = insertArgument(pargs_list, \
                                        [alt_input_json['input_prefix'], file_utils.downloadFiles(alt_input_json['input'], \
                                                                                        alt_input_json['input_directory'], \
                                                                                        file_utils.inferFileSystem(alt_input_json['input_directory']), \
                                                                                        mock)], \
                                        alt_input_json['input_position'])
    ...
    for alt_output_json in mi_json['alternate_outputs']:
        pargs_list = insertArgument(pargs_list, \
                                    [alt_output_json['output_prefix'], file_utils.getFullPath(alt_output_json['output_directory'], alt_output_json['output'])], \
    ...
    if rtype[0:3].lower() == 'str':
        return ' '.join(flattenList(pargs_list))
    else:
        return flattenList(pargs_list)
